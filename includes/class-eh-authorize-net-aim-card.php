...
class Eh_Authorize_Net_Card extends Wc_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->init_settings();
        $this->enabled = $this->get_option('eh_authorize_net_card_enabled');
        $this->title = $this->get_option('eh_authorize_net_card_title') != '' ? $this->get_option('eh_authorize_net_card_title') : 'Authorize.Net Card';
        $this->eh_authorize_card_button_text = $this->get_option('eh_authorize_net_card_order_button');
        $this->eh_authorize_card_login_id = $this->get_option('eh_authorize_net_card_login_id');
        $this->eh_authorize_card_transaction_key = $this->get_option('eh_authorize_net_card_transaction_key');
        $this->eh_authorize_card_mode = $this->get_option('eh_authorize_net_card_mode');
        $this->eh_authorize_card_transaction_type = $this->get_option('eh_authorize_net_card_transaction_type');
        $this->eh_authorize_card_success_message = $this->get_option('eh_authorize_net_card_success_message');
        $this->eh_authorize_card_failure_message = $this->get_option('eh_authorize_net_card_failure_message');
    ...
    public function admin_options() {
        ?><div class="wf-banner updated below-h2">
            ...
            <ul>
                ...
                <li><strong>- Refund (both full and partial) from admin page supported.</strong></li>
                <li><strong>- Authorize.Net eCheck feature which includes eCheck payment refund.</strong ></li>
                <li><strong>- Capture Later feature.</strong ></li>
                <li><strong>- Option to enable CVV number for Credit Card while Checkout.</strong ></li>
                <li><strong>- Supports various Account Types.</strong ></li>
                <li><strong>- A dedicated overview page where you will be able to view and manage all Authorize.Net transactions.</strong ></li>
        ...
        <p><a href="https://www.xadapter.com/product/authorize-net-payment-gateway-woocommerce/" target="_blank" class="button button-primary">Upgrade to Premium Version</a> <a href="http://www.authorizenetwoodemo.extensionhawk.com/wp-admin/admin.php?page=wc-settings&tab=checkout&section=eh_authorize_net_aim_card" target="_blank" class="button">Live Demo</a></p>
        </div>
        <style>
            .wf-banner img {
                float: right;
